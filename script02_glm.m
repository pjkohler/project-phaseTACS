...
for s = 1:length(subDirs)
    ...
    [glm_results,denoised_data] = GLMdenoisedata(glm_design{s},glm_data,stimDur,trDur,'assume',[],opt,figPath);
...
for s = 1:length(subDirs)
    outName = sprintf('%s/glmDenoise',subDirs{s});
    niiPath = sprintf('%s.nii.gz',outName);
    matPath = sprintf('%s.mat',outName);
    dataFiles = subfiles(sprintf('%s/run*vr.nii.gz',subDirs{s}),1);
    ...
    afniCommand{1} = sprintf(...
        '3drefit -fbuc -redo_bstat -relabel_all_str ''beta0 snr0 beta90 snr90 beta180 snr180 beta270 snr270 modelR2 modelSNR'' %s',niiPath);
    ...
    afniCommand{2} = sprintf(...
        'source ~/.bashrc; cd %s; rm *h.glmDenoise.niml.dset; mriVol2Surf.py %s glmDenoise.nii.gz --surfvol %s_fs4_SurfVol_ns_Alnd_Exp+orig',subDirs{s},subID,subID);
